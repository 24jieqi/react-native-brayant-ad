...
| `preload.ts` | | 预加载管理（`preloadFeedAd`, `preloadBannerAd`, `preloadSplashAdV2`） |
| `splash.ts` | | `showSplashAd()` 开屏控制器，单请求互斥 + 超时 + 事件订阅 |
...
```
types.ts ← request.ts, native.ts ← preload.ts, splash.ts ← candidates.ts
                                             ↕
                                      sdk.ts ← native.ts
```
